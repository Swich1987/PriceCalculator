...
from .views import calc_price_with_discount, NO_KEY_ERR, \
    KEY_MUST_BE_DECIMAL_ERR, make_error_response, \
...
class EndpointTestCase(SimpleTestCase):
    ...
    def _check_response(self, data, expected_json):
        request = self.request_factory.post('/path/', data=data,
    ...
    def _test_wrong_decimal_param_in_request(self, data, param):
        if param:
            self._test_make_error_response(data,
                                           KEY_MUST_BE_DECIMAL_ERR.format(param)
            )
        else:
            raise AssertionError(f'No param to check in test {self.id()}.')
...
class CalcPriceWithDiscountTestCase(EndpointTestCase):
    ...
    def test_wrong_price_in_request(self):
        self._test_wrong_decimal_param_in_request(data={'price': 'test'},
                                                  param='price')

    def _check_calculation(self, base_price, expect_rate, mock_discount):
        data = {'price': base_price}

        mock_discount_1 = MagicMock(order_price=Decimal(1000),
                                    discount_rate=Decimal(0.01))
        mock_discount_2 = MagicMock(order_price=Decimal(2000),
                                    discount_rate=Decimal(0.02))
        mock_discount.objects.order_by('order_price').all.return_value = [
            mock_discount_1, mock_discount_2]

        expect_price = Decimal(base_price) * (1 - expect_rate)
        expected_json = {
            'price_with_discount': str(quantize_price(expect_price))
        }
    ...
    def test_positive_calculation(self, mock_discount):
        self._check_calculation(base_price=Decimal(999.99),
                                expect_rate=Decimal(0),
                                mock_discount=mock_discount)
        self._check_calculation(base_price=Decimal(1000.01),
                                expect_rate=Decimal(0.01),
                                mock_discount=mock_discount)
        self._check_calculation(base_price=Decimal(1999.99),
                                expect_rate=Decimal(0.01),
                                mock_discount=mock_discount)
        self._check_calculation(base_price=Decimal(2000.01),
                                expect_rate=Decimal(0.02),
                                mock_discount=mock_discount)
...
class CalcPriceWithStateTaxTestCase(EndpointTestCase):
    ...
    def test_no_state_code_in_request(self):
        self._test_no_param_in_request(data={'price': Decimal(1000)},
                                       param='state_code')

    def test_wrong_price_in_request(self):
        self._test_wrong_decimal_param_in_request(
            data={'price': 'test', 'state_code': 'CA'}, param='price')

    def _check_calculation(self, base_price, expect_rate, mock_state_tax):
        state_code = 'CA'
        data = {'price': base_price, 'state_code': state_code}

        mock_state_tax.objects.get(state_code=state_code).tax_rate = Decimal(
            expect_rate)

        expect_price = Decimal(base_price) * (1 + expect_rate)
        expected_json = {
            'price_with_state_tax': str(quantize_price(expect_price))
        }
