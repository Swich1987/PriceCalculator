...
def calc_price_with_discount(request):
    ...
    for discount in Discount.objects.order_by('order_price').all():
        ...
        discount_rate = discount.discount_rate / 100

    price_with_discount = base_price * (1 - discount_rate)

    return JsonResponse({"price_with_discount": price_with_discount})
...
def calc_price_with_state_tax(request):
    ...
    tax_rate = StateTax.objects.get(state__code=state_code).tax_rate / 100
    price_with_state_tax = base_price * (1 - tax_rate)

    return JsonResponse({"price_with_state_tax": price_with_state_tax})
