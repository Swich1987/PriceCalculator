...
NO_KEY_ERR = 'No "{}" key.'
KEY_MUST_BE_DECIMAL_ERR = '"{}" must be decimal number.'
...
def quantize_price(dec: Decimal) -> Decimal:
...
def calc_price_with_discount(request):
    ...
    try:
        ...
        return make_error_response(NO_KEY_ERR.format("price"))
...
def calc_price_with_state_tax(request):
    ...
    try:
        ...
        return make_error_response(NO_KEY_ERR.format(err.args[0]))
